...
export default function Navbar() {
  ...
  // Improved scroll functionality with better thresholds
  useEffect(() => {
    let lastScrollY = window.pageYOffset;
    ...
    const handleScroll = () => {
      window.requestAnimationFrame(() => {
        const currentScrollY = window.pageYOffset;
        // Increased threshold for more stable behavior
    ...
    <div className="sticky top-6 z-40 mt-6">
      ...
      <div className="md:hidden relative">
        <div 
          ref={mobileNavRef}
          className={`
            bg-slate-100 h-[60px] mx-auto border border-black rounded-full 
            transition-all duration-700 ease-in-out relative overflow-hidden
            ${scrolled ? 'w-[280px]' : 'w-[350px]'}
          `}
        >
          {/* Normal State - Logo + Menu */}
          <div className={`
            absolute inset-0 flex items-center justify-between px-6 transition-all duration-700 ease-in-out
            ${scrolled ? 'opacity-0 pointer-events-none transform translate-y-2' : 'opacity-100 pointer-events-auto transform translate-y-0'}
          `}>
            <img
              src="./heart.png"
              alt="Logo"
            ...
            <button
              onClick={() => setOpen(!open)}
              className="nav-content inline-flex items-center justify-center p-2 rounded-md text-gray-600 hover:text-blue-600 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500 transition-all duration-300 flex-shrink-0 opacity-0"
            >
              {open ? <X size={24} /> : <Menu size={24} />}
            </button>
          </div>

          {/* Scrolled State - Available for work */}
          <div className={`
            absolute inset-0 flex items-center justify-center transition-all duration-700 ease-in-out
            ${scrolled ? 'opacity-100 pointer-events-auto transform translate-y-0' : 'opacity-0 pointer-events-none transform -translate-y-2'}
          `}>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
              <span className="text-gray-800 font-medium text-sm whitespace-nowrap">
        ...
        {/* Mobile Menu Dropdown */}
        <div className={`
          absolute top-[68px] left-1/2 transform -translate-x-1/2 w-full max-w-[380px] 
          bg-white/90 backdrop-blur-md border border-black rounded-2xl overflow-hidden z-50
          ...
          <div className="py-4 space-y-2">
            {navItems.map((item, index) => (
              <a
                ...
                onClick={() => setOpen(false)}
                className={`
                  block text-gray-600 hover:text-blue-600 px-6 py-3 text-base font-medium 
                  transition-all duration-300 hover:bg-gray-100/50 hover:translate-x-2
            ...
            <div className={`
              px-6 pt-2 transition-all duration-300
              ${open ? 'opacity-100 transform translate-x-0' : 'opacity-0 transform translate-x-4'}
            `}
            style={{ transitionDelay: `${navItems.length * 100}ms` }}
            >
              <button className="w-full bg-gray-700 px-4 py-3 rounded-full text-white hover:bg-gray-800 transition-all duration-300 hover:scale-105">
                Contact
              </button>
            </div>
          </div>
