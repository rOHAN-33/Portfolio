...
const ImageGrid = () => {
  ...
  useEffect(() => {
    ...
    if (isDesktop) {
      ...
      // Fixed Mobile animations
      // Check if mobile refs exist before setting initial states
      if (mobileNameRef.current) {
        gsap.set(mobileNameRef.current, { y: 30, opacity: 0 });
      }
      if (mobileCreativeRef.current) {
        gsap.set(mobileCreativeRef.current, { y: 30, opacity: 0 });
      }
      if (mobileDeveloperRef.current) {
        gsap.set(mobileDeveloperRef.current, { y: 30, opacity: 0 });
      }
      if (mobileRotatingTextRef.current) {
        gsap.set(mobileRotatingTextRef.current, { y: 30, opacity: 0 });
      }
      if (mobileImageRef.current) {
        gsap.set(mobileImageRef.current, { scale: 1.1, opacity: 1 });
      }
      if (mobileImageRevealRef.current) {
        gsap.set(mobileImageRevealRef.current, { clipPath: 'inset(100% 0 0 0)' });
      }
      if (mobileCircleRef.current) {
        gsap.set(mobileCircleRef.current, { scale: 0, opacity: 0 });
      }

      // Mobile animation sequence - step by step
  ...
  return (
    <div ref={containerRef} className='min-h-screen flex items-center justify-center overflow-y-hidden px-4 py-16 md:py-20 lg:mt-[-40px]'>
      {/* Desktop Layout */}
